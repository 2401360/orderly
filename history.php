...
<div class="container py-4">
  ...
  $sqlCount = $pdo->prepare('SELECT COUNT(*) FROM purchase WHERE customer_id = ?');
  $sqlCount->execute([$_SESSION['customer']['id']]);
  $totalOrders = (int)$sqlCount->fetchColumn();
  $totalPages = max(1, (int)ceil($totalOrders / $perPage));
  $sql = $pdo->prepare('SELECT id, created_at FROM purchase WHERE customer_id = ? ORDER BY id DESC LIMIT ? OFFSET ?');
  ...
  foreach ($purchases as $purchase):
    $pid = (int)$purchase['id'];
    $sql2 = $pdo->prepare('SELECT product.id, product.name, product.price, purchase_detail.count FROM purchase_detail JOIN product ON product.id = purchase_detail.product_id WHERE purchase_detail.purchase_id = ?');
    $sql2->execute([$pid]);
    $total = 0;
  ?>
    <div class="card mb-4">
          ...
          <table class="table table-striped align-middle">
            ...
            <tbody>
              ...
              ?>
                <tr>
                  <td><?= (int)$row['id'] ?></td>
                  <td><a href="detail.php?id=<?= (int)$row['id'] ?>"><?= htmlspecialchars($row['name']) ?></a></td>
