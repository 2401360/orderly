...
<div id="main-content">
                ...
                <div class="slider-mover" id="sliderMover">
                    <?php foreach ($recommended as $p): ?>
                        <a href="detail.php?id=<?= (int)$p['id'] ?>" class="slide-item">
                            <img src="<?= e($p['image_url']) ?>" alt="<?= e($p['name']) ?>" class="slide-img">
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
    ...
    <section class="chef-section">
        ...
        <div class="chef-text">
            <h2>当店のこだわり</h2>
            <p>素材の選び抜きから焼成時の温度管理、そして繊細なデコレーションに至るまで、すべての工程に一切の妥協を許さず、ひとつひとつのケーキを丹念に仕上げております。旬の果実や地元産の良質な素材を贅沢に使用し、四季の移ろいを感じられるラインナップをご用意しております。さらに、味わいだけでなく、思わず目を奪われるような美しさにもこだわり、お客様に驚きと感動のひとときをお届けできるよう心を尽くしております。職人の情熱を込めた、上質なケーキの数々をぜひご堪能ください。</p>
        </div>
        <div class="chef-side">
    ...
    <div class="melt-decoration">
        <svg viewBox="0 0 1440 200" preserveAspectRatio="none">
            <path d="M0,40 C120,80 240,120 360,100 C480,80 600,40 720,70 C840,100 960,160 1080,140 C1200,120 1320,80 1440,100 L1440,200 L0,200 Z"></path>
        </svg>
    </div>
    <section class="section" style="padding:50px 20px; background:#F8F1E7;">
        <div class="carousel-reco-grid" style="display:grid;grid-template-columns:1fr 1fr; align-items:center; max-width:1200px; margin:0 auto; position:relative; gap:20px;">
                ...
                <div id="recoCarousel" class="carousel slide mb-4" data-bs-ride="carousel">
                    <div class="carousel-indicators">
                        <?php foreach ($recommended as $index => $item): ?>
                            ...
                            <button type="button" data-bs-target="#recoCarousel" data-bs-slide-to="<?= $index ?>" class="<?= $index === 0 ? 'active' : '' ?>"></button>
                        <?php endforeach; ?>
                    </div>
                            ...
                            <div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
                                <a href="detail.php?id=<?= (int)$item['id'] ?>">
                                    <?php if (!empty($item['image_url'])): ?>
                                        <img src="<?= e($item['image_url']) ?>" class="d-block w-100" style="aspect-ratio:2/1;object-fit:cover;">
                                    <?php else: ?>
                                        <div class="bg-secondary d-flex align-items-center justify-content-center text-white" style="aspect-ratio:2/1;">No Image</div>
                                    <?php endif; ?>
                                </a>
                            </div>
                        <?php endforeach; ?>
                    </div>
                    <button class="carousel-control-prev" type="button" data-bs-target="#recoCarousel" data-bs-slide="prev"><span class="carousel-control-prev-icon"></span></button>
                    <button class="carousel-control-next" type="button" data-bs-target="#recoCarousel" data-bs-slide="next"><span class="carousel-control-next-icon"></span></button>
                </div>
            </div>
            <div style="position:relative; z-index:2; text-align:center;">
                ...
                <h2 style="font-size:3rem;font-weight:700;color:#4A3B2A;font-family:'Noto Serif JP',serif;">季節のおすすめ</h2>
                <svg width="150" height="50" style="margin-top:10px;">
                    <path d="M0,25 Q75,0 150,25" stroke="#4A3B2A" stroke-width="2" fill="none"></path>
                    <path d="M0,25 Q75,50 150,25" stroke="#4A3B2A" stroke-width="2" fill="none"></path>
                </svg>
            </div>
            <div style="position:absolute;top:0;left:50%;width:50%;height:100%;background:linear-gradient(to right, rgba(248,241,231,0) 0%, #F8F1E7 100%);pointer-events:none;z-index:1;"></div>
        </div>
    </section>
</div>
...
<script>
    ...
    const shouldPlay = !lastPlayed || (now - Number(lastPlayed) > LIMIT);
    if (shouldPlay) {
